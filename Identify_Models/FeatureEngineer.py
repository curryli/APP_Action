...
def preprocess(file_dir, Seg_granularity):
    ...
    with open(file_dir) as f:
        ...
        for line in f:
            ...
            if len(raw_list) < 5:
                continue
            if raw_list[3]=='' or raw_list[4]=='' or raw_list[5]=='':
                continue
            print index
            status  = raw_list[1]  #行为类型 5类
    ...
    for gravity_tuple in gravity_data:
        if (counter >= Seg_granularity or gravity_tuple["status"] != last_status):
    ...
    with open("WISDM_FE_save.csv", "w") as the_file:  #手动添加表头label,max,min,std,mean
...
if __name__ == "__main__":
    preprocess("data/WISDM_ar_v1.1_modify.txt", 100)
    pass